...
if ii==4 || ii>=6
ans = dlmread(['\\gs\StPetersburgFL-G\NACCH\Imagery\madbeach\surveys\walking\20220915\line60.xyz']); %%%%%%%changing to 9/15/22 for testing
...
mult=find(a>1);
while size(profile.z) ~= size(unique(profile.z)) % wasnt catching >2 identical values
    for i=1:length(mult)
