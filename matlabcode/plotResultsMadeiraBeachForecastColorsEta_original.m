...
year  = 2020;
month = 11;  monthNum=num2str(month,'%02d');
day   = 11;  dayNum=num2str(day,'%02d');
hr1   = 16;
% hr2  = 18;
...
geom = load('\\gs\StPetersburgFL-G\NACCH\Imagery\madbeach\calibration\extrinsic\20170217\orig\geomFile_20170217.mat');
...
surveys = [];
for ii=1:length(surveys_all); surveys = [surveys; surveys_all(ii).name];end
tmp=((datenum(year,month,day)-datenum(surveys,'yyyymmdd')));
ii=find(tmp>0,1,'last');
if ii==4 || ii>=6
dlmread(['\\gs\StPetersburgFL-G\NACCH\Imagery\madbeach\surveys\walking\',surveys(ii,:),'\line60.xyz']);
profile.E=ans(:,1);profile.N=ans(:,2);
[profile.x,profile.y]=coordSys_madbeach(profile.E,profile.N);
profile.z=ans(:,3);
elseif ii==1 || ii==2 || ii==3 || ii==5
dlmread(['\\gs\StPetersburgFL-G\NACCH\Imagery\madbeach\surveys\walking\',surveys(ii,:),'\line49.xyz']);
p1.E=ans(:,1);p1.N=ans(:,2);
[p1.x,p1.y]=coordSys_madbeach(p1.E,p1.N);
p1.z=ans(:,3);
dlmread(['\\gs\StPetersburgFL-G\NACCH\Imagery\madbeach\surveys\walking\',surveys(ii,:),'\line48.xyz']);
...
[a,b]=histc(profile.z,unique(profile.z));
mult=find(a>1);
while ~isempty(mult) % wasnt catching >2 identical values
    for ii=1:length(mult)
        index=find(ismember(b, mult(ii)));
        profile.z(index(2))=profile.z(index(2))+0.0000000001;
    end
    [a,b]=histc(profile.z,unique(profile.z));
    mult=find(a>1);
end
...
print('-dpng', ['C:\Users\mpalmsten\OneDrive - DOI\projects\TWL\MadeiraBeach\ForecastOnImage\TimexTWLForecast',num2str(year), num2str(month), num2str(day), num2str(hr1), '.png'])
...
axis image; axis(ax);
%title([datestr(R.time(tindex)) 'GMT']);
print('-dpng', 'C:\Users\mpalmsten\OneDrive - DOI\projects\TWL\MadeiraBeach\ForecastOnImage\TimexTWLForecast2020091616.png')
