...
%--------------------------------------------------------------------------
function computeRunupElevation_fromTimestacks_madbeach(runupFile,topo)
...
for ss=1:length(runupData)
    ...
    yloc    = str2double(runupName.type(2:end)); %alongshore location of pixel transect
    ...
    [~,~,~,~,Runup.X2,~,~,~,~,~,~,~] = runupMaxValueStats(-Runup.x(good),tRunup);
    [~,~,~,~,Runup.Xi2,~,~,~,~,~,~,~] = runupMaxValueStats(-Runup.xi(good),tRunup);
    ...
    %iloc=find(min(abs(topo.y(:,1)-(-Runup.yloc)))==abs(topo.y(:,1)-(-Runup.yloc)));
    ...
    [Runup.slope]      = calcRunupLocalBeachSlope([Runup.ts(good), Runup.zi(good)],Profile);
